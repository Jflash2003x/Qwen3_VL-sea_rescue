...
def plot_overall(b, v, path):
    ...
    b2 = ax.bar(x + w/2, vv, w, label='V2 微调', color=C_V2, edgecolor='white')
    for bar in b1:
...
def plot_category_recall(b_cat, v_cat, path):
    ...
    ax.bar(x + w/2, vv, w, label='V2 微调', color=C_V2, edgecolor='white')
...
def plot_f1_curve(base_results, v2_results, path):
    ...
    ax.plot(ious, vf1, 's-', color=C_V2, linewidth=2.5, markersize=7, label='V2 微调')
...
def plot_improvement(b, v, path):
    ...
    ax.set_title(f'V2 微调 vs 原始模型 提升 (IoU@{MAIN_IOU})', fontsize=15, fontweight='bold', **fkw())
...
def plot_detections(gt_data, base_preds, v2_preds, img_list, charts_dir, n=8):
    ...
    for idx, (img_name, gts, bp, vp, diff, ng, b_tp, v_tp) in enumerate(selected):
        ...
        draw(axes[0], gts, f'GT ({len(gts)}个目标)')
        draw(axes[1], bp, f'原始模型 ({len(bp)}个预测) TP={b_tp}')
        draw(axes[2], vp, f'V2 微调 ({len(vp)}个预测) TP={v_tp}')
